...
class Solution:
    def getIntersectionNode(self, headA: ListNode, headB: ListNode) -> ListNode:
        ...
        while p1 is not p2:
            #不断向前移动指针，遍历p1以及p2
            #若是两个链表的长度相等，第一遍遍历则可以得到相交节点
            #若是不相等，较短的一方则会先进入被交换赋值
            #而在此时交换双方，则相当于补全较短一方的长度与较长一方相同，再次遍历。
            #所以若相交，第二次必然得到相交结点，否则p1，p2将遍历赋值为空。
            #注：在不相等前提下，当P1与P2为同一链表时，必然不会有相交节点，因为其指针指向必然不相同。
